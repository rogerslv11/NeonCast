        ...
        <div className="relative z-10 flex flex-col justify-between p-12 w-full">
          ...
          <div className="max-w-md">
            <h2 className="text-4xl font-bold text-white leading-tight mb-4">
              Séries em áudio que dão vida
              <br />
              <span className="text-accent">a cada história</span>
            </h2>
            <p className="text-sm text-white/50 leading-relaxed">
              Mais de 10 séries originais com narração profissional. Ouça quando e onde quiser.
            </p>
          </div>
          ...
          <div className="flex items-center gap-8">
            {[
              { value: '10+', label: 'Séries' },
              { value: '100+', label: 'Episódios' },
              { value: '8', label: 'Narradores' },
            ].map((stat) => (
              <div key={stat.label}>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center px-6">
        <div className="w-full max-w-sm">
          ...
          <div className="mb-10">
            ...
            <p className="text-sm text-text-muted/60">Entre para continuar ouvindo</p>
          </div>
            ...
            >
              {isLoading === 'GitHub' ? (
                ...
                <>
                  ...
                  <span className="flex-1 text-left">Continuar com GitHub</span>
                  <svg className="w-4 h-4 text-text-muted/30 group-hover:text-text-muted/50 group-hover:translate-x-0.5 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
                  </svg>
                </>
          ...
          <div className="mb-8">
            <div className="flex items-center gap-3 mb-3">
              <div className="flex-1 h-px bg-border/40" />
              <span className="text-[10px] text-text-muted/30 uppercase tracking-wider">Admin</span>
              <div className="flex-1 h-px bg-border/40" />
            </div>
            ...
            <button
              ...
              disabled={isLoading !== null}
              className="group w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl border border-accent/20 bg-accent/5 text-accent text-sm font-medium hover:bg-accent/10 hover:border-accent/30 transition-all active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed disabled:active:scale-100"
            >
              {isLoading === 'admin' ? (
          ...
          <p className="text-center text-sm text-text-muted/50">
            Não tem conta?{' '}
            <a href="/auth/register" className="text-text-primary hover:text-accent font-medium transition-colors inline-flex items-center gap-1">
              Criar conta grátis
